...
const MESSAGES = {
  en_US: {
    ...
    errorExistingContentInInsertCellRange:
      'Existing Content in Insert Cell Range: Check the selected range.',
    alertAlreadySetupMessage:
      'Initial settings are already complete. Do you want to overwrite the settings?\n\n',
    promptCurrentValue: '\n\nCurrent Value: {{value}}',
    errorCanceled: 'Canceled.',
    promptFolderId:
      'Google Drive folder ID to get the images from. Enter "root" if you want to designated your Google Drive\'s root folder.',
    promptFileExt:
      'File extension of the image file(s) without the period. e.g., NOT ".jpg" but "jpg"',
    promptSelectionVertical:
      'selectionVertical: Enter "true" or "false". When true, the script will assume that the cells are selected vertically, i.e., in a single column.',
    promptInsertPosNext:
      'insertPosNext: Enter "true" or "false". When true, the images will be inserted in the next row or column, depending on the value of selectionVertical.',
  ...
  ja: {
    ...
    errorInitialSettingNotComplete:
      '初期設定が完了していません。メニュー「Googleドライブから画像挿入」から「初期設定」を実行してください。',
    alertMessageOnCompleteTitle: '画像挿入 完了',
    alertMessageOnComplete:
      '挿入された画像が見えない場合は、スプレッドシートのウェブページを更新してください。\n\nGoogleドライブからの画像取得：{{getBlobsCompleteSec}}秒\n処理全体は{{insertImageCompleteSec}}秒で完了\n\n',
    alertMessageAdd:
      '{{key}}: 同じ名前の画像ファイルが{{value}}個、見つかりました。',
    errorMoreThanOneColumnSelected:
      '複数の列が選択されています。選択範囲をご確認ください。',
    errorMoreThanOneRowSelected:
      '複数の行が選択されています。選択範囲をご確認ください。',
    errorEmptyCellsSelected: '選択範囲が空白です。ご確認ください。',
    errorImageFileFormat:
      '画像ファイルの形式を確認してください。 {{fileName}}\n画像ファイルはJPG、PNG、またはGIFのいずれかの形式である必要があります。',
    errorImageFileSizeExceedsLimit:
    ...
    alertAlreadySetupMessage:
      '初期設定はすでに完了しています。既存の設定を上書きしますか？',
    promptCurrentValue: '\n\n現在の値: {{value}}',
    errorCanceled: 'キャンセルされました。.',
    promptFolderId:
      '画像が保存されているGoogleドライブのフォルダID。「マイドライブ」直下を指定したい場合は「root」と入力してください。',
    promptFileExt:
      '画像ファイルの拡張子。ピリオド（.）は除いてください。\n例）「jpg」→○、「.jpg」→×',
    promptSelectionVertical:
      'selectionVertical：選択範囲の縦横方向を指定。「true」（縦方向）または「false」（横方向）を入力。',
    promptInsertPosNext:
      'insertPosNext：画像を挿入する位置を指定。「true」（選択範囲の方向によって右列または下行）または「false」（選択範囲の方向によって左列または上行）を入力。',
...
class LocalizedMessage {
   ...
   * @returns {string} The replaced text.
   */
  replaceErrorUnknownError(
    selectedRangeA1Notation,
    optionsSelectionVertical,
    rangeNumRows,
    rangeNumColumns
  ) {
